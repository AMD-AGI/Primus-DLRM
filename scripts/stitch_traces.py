#!/usr/bin/env python3
"""Stitch per-rank Chrome traces from a Primus-DLRM run into one merged file.

The trainer's tracer (when ``--trace-ranks`` selects more than one rank)
writes one file per rank::

    <run>/trace/trace_step{step}_rank{rank}.json
...
# trace_step52_rank3.json or trace_3_rank0.json (legacy format from Tracer)
...
def _process_one_rank(
    ...
    """Filter + remap one rank's events. ``include`` ⊆ {'cpu','gpu','other'}."""
